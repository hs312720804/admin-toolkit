...
export default {
  ...
  mounted () {   
    ...
    let that = this;   
    document.body.addEventListener('click', () => {//鼠标单击组件之外时收起下拉列表
      that.isFocus = false;
      that.value = "";
    }, false);
  },
  ...
  methods: {
    ...
    clickSearch () {//回车后查询
      ...
      this.selectItem = {};//选中的搜索结果
      if (this.value.trim()) {                                 
        for (let i = 0; i < this.FuzzySearchList.length; i++) {//根据所有人员名字匹配
    ...
    getKeyWord: function (name, search) {//关键字
      let keyword = [];
    ...
    clickResultItem: function (data) {//单击下拉列表中的选项    
