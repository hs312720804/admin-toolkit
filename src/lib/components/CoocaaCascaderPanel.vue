<template>
  <div @click.stop="" class="search-selector">
      <div class="search-selector-tags--bottom search-selector-tags" v-if="position === 'top'">
...
export default {
  ...
  methods: {
    ...
    setSelectedNodes () {
      ...
      this.$emit('get-all-selected-nodes', this.selectedAllNodes)
    },
    /*
    得到选中的节点，如果有父节点在，其选中的子节点去掉
    */
    getSelectedNodes (selectedAllNodes, callbackText) {
    ...
    /*
      编辑和查看时候设置组件选中的子节点
     */
    setLeaf (parentIdArr, options) {
      const data = this.transformOptions(options)
      let fun = (value, init) => {
    ...
    emitInputValue () {
      this.$nextTick(() => {
        if (this.$refs.panel) {
          ...
          this.selectedItem = this.getSelectedNodes(this.selectedAllNodes, true)
          this.$emit('get-selected-list', this.selectedItem)
        }
      })
    },
