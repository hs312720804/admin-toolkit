...
function QueryEls (selector, context) {
  // 选择器
  if (!selector) { this.els = [] } else if (typeof selector === 'string') { this.els = (context || document).querySelectorAll(selector) } else if (selector.addEventListener) { this.els = [selector] } // 单个dom元素
  else { this.els = selector } // dom元素集合
